...
/// @title MiniMeToken Contract, Updated Reputation Version
/// @author Jordi Baylina, modified by Kurt :)
/// @dev This token contract's goal is to make it easy for anyone to clone this
///  token using the token distribution at a given block, this will allow DAO's
///  and DApps to upgrade their features in a decentralized manner without
///  affecting the original token    /// @return True if the transfer was successful
...
contract MiniMeToken is Controlled {
    ...
    string public name;                //The Token's name: e.g. Reputation Token
    uint8 public decimals;             //Number of decimals of the smallest unit
    string public symbol;              //An identifier: e.g. REP
    string public version = 'MMT_REP_0.1'; //An arbitrary versioning scheme
...
////////////////
    ...
    /// @notice mints `_amount` tokens that are assigned to `_recipient`
    /// @param _recipient The address that will be assigned the new tokens
    ...
    /// @param _amount The quantity of tokens generated
    /// @return True if the tokens are generated correctly
    function _mint(address _recipient, uint _amount) internal returns(bool) {
        uint curTotalSupply = totalSupply();
        uint previousBalanceTo = balanceOf(_recipient);
        updateValueAtNow(totalSupplyHistory, curTotalSupply + _amount);
        updateValueAtNow(balances[_recipient], previousBalanceTo + _amount);
    ...
    function _burn(address _owner, uint _amount) internal returns(bool) {
        uint curTotalSupply = totalSupply();
        require(curTotalSupply >= _amount, "Amount greater than total supply");
        uint previousBalanceFrom = balanceOf(_owner);
        require(previousBalanceFrom >= _amount, "Amount must be less than users balance");
        updateValueAtNow(totalSupplyHistory, curTotalSupply - _amount);
...
////////////////
    ...
    /// @param _transfersEnabled True if transfers are allowed in the clone
    function enableTransfers(bool _transfersEnabled) external virtual onlyController {
        transfersEnabled = _transfersEnabled;
    }
...
////////////////
    ...
    function getValueAt(Checkpoint[] storage checkpoints, uint _block
        ...
        uint _min = 0;
        uint _max = checkpoints.length-1;
        while (_max > _min) {
            uint mid = (_max + _min + 1)/ 2;
    ...
    function updateValueAtNow(Checkpoint[] storage checkpoints, uint _value
        ...
        || (checkpoints[checkpoints.length -1].fromBlock < block.number)) {
               Checkpoint memory newCheckPoint = Checkpoint(uint128(block.number), uint128(_value));
