...
contract ReputationTokenFactory {
    ...
    function create(string memory _name, string memory _symbol, uint8 _decimals, address _owner, bool _transferable, bool _burnable) external returns(address) {
        ReputationToken newToken = new ReputationToken(_name, _symbol, _decimals, _owner, _transferable, _burnable);
